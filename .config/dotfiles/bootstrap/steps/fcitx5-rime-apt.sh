#!/usr/bin/env bash
# Debian/Ubuntu：安装配置 fcitx5 + rime，写入环境变量（不含 rime-ice 自动安装）
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$SCRIPT_DIR/lib/common.sh"

echo "[fcitx5-rime][apt] apt 更新与安装 fcitx5 组件"
sudo apt-get update -y
sudo apt-get install -y --no-install-recommends \
  fcitx5 fcitx5-configtool fcitx5-frontend-gtk4 fcitx5-frontend-gtk3 fcitx5-frontend-qt5 \
  fcitx5-rime fcitx5-material-color

echo "[fcitx5-rime][apt] rime-ice 无官方包，如需请自行提供源或使用手动资源包（脚本不自动安装）"

echo "[fcitx5-rime][apt] 写入 /etc/environment 输入法环境变量（追加/覆盖同名键）"
tmp_env="$(mktemp)"
...
echo "[fcitx5-rime][apt] 准备用户 rime 配置目录并写入 default.custom.yaml"
...
echo "4) 如需 rime-ice，请自行添加源或使用手动包（脚本未自动安装）。"
